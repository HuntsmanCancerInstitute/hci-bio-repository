...
When mounting new volumes, AWS credentials need to provided. By default, 
this uses your AWS credentials. Be sure to specify your profile. Buckets
must be preconfigured with appropriate permissions. Mounted volume 
names may only contain letters and underscore (no dashes) and are limited to
32 characters. Names may be autogenerated from the bucket name, but to avoid
surprises, provide your own.
