...
unless (@ARGV) {
  ...
  A script to summarize one or more lists generated by the app
  manage_repository.pl. The lists should be existing files, or the
  special file 'stdin' can be provided, where it will read from STDIN.
  ...
  USAGE:
    ...
    manage_repository.pl [options] --status | summarize_lists.pl stdin
...
sub format_size {
	...
	if ($number > 1099511627776) {
		return sprintf("%.1f TiB", $number / 1099511627776);
	}
	elsif ($number > 1073741824) {
		return sprintf("%.1f GiB", $number / 1073741824);
	}
	elsif ($number > 1048576) {
		return sprintf("%.1f MiB", $number / 1048576);
	}
	elsif ($number > 1000) {
		# avoid weird formatting situations of >1000 and < 1024 bytes
		return sprintf("%.1f KiB", $number / 1024);
	}
	else {
